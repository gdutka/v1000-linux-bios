      ...
      if (IsString) {
        ...
        if (Length < ((LengthStr / 4 + 1) * 2)) {
          ...
          for (Index = 0; Index < LengthStr; Index += 4) {
            StrnCpyS (TemStr, ARRAY_SIZE(TemStr), Value + Index, 4);
  ...
  )
{
  ...
  if ((Action > 0) && ((Question->QuestionFlags & EFI_IFR_FLAG_CALLBACK) != 0) && (ConfigAccess != NULL)) {
    ...
    //
    // Load Default F9 HotKey
    //
    if ((QuestionId == KEY_SCAN_F9) && (Action == EFI_BROWSER_ACTION_DEFAULT_MANUFACTURING)) {
      mLoadDefault  = TRUE;
    }
  ...
  )
{
  ...
  if (!EFI_ERROR (Status) && CompareGuid (FormSet->ClassGuid, &mScuFormSetGuid)) {
    //
