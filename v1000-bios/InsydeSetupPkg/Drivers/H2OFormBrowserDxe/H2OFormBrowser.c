  ...
  )
{
  ...
  while (!IsNull (&Private->FormSetList, LinkFormSet)) {
    ...
    while (!IsNull (&FormSet->FormListHead, LinkForm)) {
      ...
      if (Form->NumberOfStatement == 0) {
        continue;
      }

      DisplayOrderValueList = AllocatePool (Form->NumberOfStatement * sizeof (UINT16));
  ...
  )
{
  ...
  if (GetScuFormset () != NULL) {
    ...
    //
    DEBUG ((DEBUG_INFO, "OemKernelServices Call: OemSvcAdjustNavigationMenu \n"));
    OemSvcStatus = OemSvcAdjustNavigationMenu (SetupMenuHiiHandle, &SetupMenuCount, &ZeroGuid);
    DEBUG ((DEBUG_INFO, "OemKernelServices OemSvcAdjustNavigationMenu Status: %r\n", OemSvcStatus));
  }
...
/**
 Check if there is a storage element in the formset
  ...
  )
{
  ...
  // Check current browser formset first.
  //
  if (mSystemLevelFormSet != NULL && CompareGuid (mSystemLevelFormSet->ClassGuid, &mScuFormSetGuid) && HasStorageElement(mSystemLevelFormSet)) {
    return mSystemLevelFormSet;
  }
  ...
  while (!IsNull (&gBrowserFormSetList, Link)) {
    LocalFormSet = FORM_BROWSER_FORMSET_FROM_LINK (Link);
    if (CompareGuid (LocalFormSet->ClassGuid, &mScuFormSetGuid) && HasStorageElement(LocalFormSet)) {
      return LocalFormSet;
    }
    Link = GetNextNode (&gBrowserFormSetList, Link);
  }
