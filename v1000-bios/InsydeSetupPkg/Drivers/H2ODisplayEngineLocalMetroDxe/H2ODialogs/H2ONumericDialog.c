      ...
      L"<HorizontalLayout padding='0,0,10,0' name='DialogPasswordInput' height='51'>"
        L"<VerticalLayout padding='6,8,6,8' background-color='0xFFF2F2F2' height='41'>"
          L"<Control padding='1,1,1,1' background-color='0xFF999999' height='29'>"
            L"<HorizontalLayout>"
              L"<UiEdit font-size='19' padding='7,3,0,22' name='InputNumericText' background-color='0xFFF2F2F2' focusbkcolor='@menulightcolor'  height='27' tabstop='true' taborder='1'  valuetype='dec'/>"
              L"<Label name='Prefix' text=''  font-size='19' height='27' width='wrap_content' background-color='0x00000000' float='true' />"
            L"</HorizontalLayout>"
          L"</Control>"
        L"</VerticalLayout>"
  ...
  case UI_NOTIFY_WINDOWINIT:
    ...
    if (mFbDialog->BodyInputCount != 0) {
      ...
      if ((CurrentQ->Flags & EFI_IFR_DISPLAY) == EFI_IFR_DISPLAY_UINT_HEX) {
        ...
        Control = UiFindChildByName (Dialog, L"Prefix");
        if (Control != NULL) {
  ...
  case UI_NOTIFY_WINDOWINIT:
    ...
    if (mFbDialog->BodyInputCount != 0) {
      ...
      if ((mFbDialog->H2OStatement->Flags & EFI_IFR_DISPLAY) == EFI_IFR_DISPLAY_UINT_HEX) {
        ...
        UiSetAttribute (Control, L"valuetype", L"dec");
        UiSetAttribute (Control, L"padding",   L"0,0,0,0");
        Control = UiFindChildByName (Dialog, L"Prefix");
        if (Control != NULL) {
