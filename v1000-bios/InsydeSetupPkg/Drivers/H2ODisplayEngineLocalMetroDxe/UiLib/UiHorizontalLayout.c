  ...
  )
{
  ...
  if ((Control->FixedSize.cx >= 0 || Control->FixedSize.cx == MATCH_PARENT) &&
      (Control->FixedSize.cy >= 0 || Control->FixedSize.cy == MATCH_PARENT)) {
  ...
  if (Control->FixedSize.cy > 0) {
    ItemAvailableSize.cy = Control->FixedSize.cy - Control->Padding.top  - Control->Padding.bottom;
  }
