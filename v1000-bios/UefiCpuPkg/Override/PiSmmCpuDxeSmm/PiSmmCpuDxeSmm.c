...
//[-start-160920-IB05960395-add]//
#include <Protocol/AcpiEnableCallbackDone.h>
//[-end-160920-IB05960395-add]//
...
/**
  Acpi Enable Callback Start event notification handler.

  It try to perform ConfigSmmCodeAccessCheck() once.
...
EFI_STATUS
EFIAPI
ConfigSmmCodeAccessCheckWrapper (
...
//[-end-160920-IB05960395-add]//
  ...
  //
  // register Smm Exit Boot Service Protocol notification
  //
  Status = gSmst->SmmRegisterProtocolNotify (
                    &gAcpiEnableCallbackStartProtocolGuid,
                    ConfigSmmCodeAccessCheckWrapper,
                    &RegistrationAcpiEnable
                    );
  ASSERT_EFI_ERROR (Status);
  ...
  )
{
  if (mSmmReadyToLock) {
    ...
    //
    //  Move the smm code access check feature from smm ready to lock event callback
    //  function to enable ACPI callback function. It will allow BIOS could access
    //  none smm code when a SMI is triggered after smm ready to lock, such as in
    //  shell environment.
    //
