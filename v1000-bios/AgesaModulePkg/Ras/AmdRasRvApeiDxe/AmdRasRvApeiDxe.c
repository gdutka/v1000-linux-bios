...
#define RAS_SYNCFLOOD_STATUS                (0x13B203c0)
/*----------------------------------------------------------------------------------------
  ...
  )
{
  ...
  if (RasMcaErrorInfo->CpuInfo.ProcessorNumber > mAmdRasPolicy->TotalNumberOfProcessors) {
	DEBUG((EFI_D_ERROR, "[RAS] AmdSearchMcaError return EFI_INVALID_PARAMETER \n"));
  ...
  if (!CpuMapFound) {
	  DEBUG((EFI_D_ERROR, "[RAS] AmdSearchMcaError return EFI_NOT_FOUND \n"));
...
//[-start-240426-IB18160212-modify-]//
  ...
  if (RasMcaErrorInfo->CpuInfo.ProcessorNumber == 0) {
    CollectMcaErrorInfo((VOID*)RasMcaErrorInfo);
  } else {
...
DumpMcaErrorInfo ()
{
  ...
  DEBUG((EFI_D_ERROR, "[RAS] PMxc0 = 0x%08x\n", MmioRead32(0xfed803c0)));
  for (i = 0; i < BankNum; i++) {
...
//[-start-240426-IB18160212-modify-]//
  ...
  Status = gBS->AllocatePool (
                              ...
                              (VOID**)&AmdRasApeiProtocol               // OUT VOID **Buffer
                              );
  if (EFI_ERROR (Status)) {
  ...
  // Get AMD Ras Policy
  Status = gBS->LocateProtocol (&gAmdRasInitDataProtocolGuid, NULL, (VOID**)&mAmdRasPolicy);
  if (EFI_ERROR (Status)) {
    ASSERT_EFI_ERROR (Status);
    return Status;    // Error detected while trying to locate pool
  }
//[-end-240426-IB18160212-modify-]//
  ...
  RasSmnRead(0, RAS_SYNCFLOOD_STATUS, &RasValue);
  if (RasValue)
  DEBUG((EFI_D_ERROR, "[RAS]  RAS_SYNCFLOOD_STATUS = 0x%08x\n", RasValue)); 
  RasSmnRead(0, 0x13B20028, &RasValue);
  if (RasValue)
  DEBUG((EFI_D_ERROR, "[RAS]  RAS_PARITY_STATUS1 = 0x%08x\n", RasValue));
  RasSmnRead(0, 0x13B2002c, &RasValue);
  if (RasValue)
  DEBUG((EFI_D_ERROR, "[RAS]  RAS_PARITY_STATUS2 = 0x%08x\n", RasValue));
  RasSmnRead(0, 0x13B20030, &RasValue);
  if (RasValue)
  DEBUG((EFI_D_ERROR, "[RAS]  RAS_PARITY_STATUS3 = 0x%08x\n", RasValue));
  RasSmnRead(0, 0x13B20034, &RasValue);
  if (RasValue)
  DEBUG((EFI_D_ERROR, "[RAS]  RAS_PARITY_STATUS4 = 0x%08x\n", RasValue));
  RasSmnRead(0, 0x13B20038, &RasValue);
  if (RasValue)
  DEBUG((EFI_D_ERROR, "[RAS]  RAS_PARITY_STATUS5 = 0x%08x\n", RasValue));
  RasSmnRead(0, 0x13B2003c, &RasValue);
  if (RasValue)
  DEBUG((EFI_D_ERROR, "[RAS]  RAS_COR_PARITY_STATUS1 = 0x%08x\n", RasValue));
  RasSmnRead(0, 0x13B20040, &RasValue);
  if (RasValue)
  DEBUG((EFI_D_ERROR, "[RAS]  RAS_COR_PARITY_STATUS2 = 0x%08x\n", RasValue));
    RasSmnRead(0, 0x13B20044, &RasValue);
  if (RasValue)
  DEBUG((EFI_D_ERROR, "[RAS]  RAS_COR_PARITY_STATUS3 = 0x%08x\n", RasValue));
    RasSmnRead(0, 0x13B20048, &RasValue);
  if (RasValue)
  DEBUG((EFI_D_ERROR, "[RAS]  RAS_COR_PARITY_STATUS4 = 0x%08x\n", RasValue));
    RasSmnRead(0, 0x13B2004c, &RasValue);
  if (RasValue)
  DEBUG((EFI_D_ERROR, "[RAS]  RAS_COR_PARITY_STATUS5 = 0x%08x\n", RasValue));
    RasSmnRead(0, 0x13B20064, &RasValue);
  if (RasValue)
  DEBUG((EFI_D_ERROR, "[RAS]  RAS_PARITY_ERROR_STATUS_UCP_GRP1 = 0x%08x\n", RasValue));
    RasSmnRead(0, 0x13B20068, &RasValue);
  if (RasValue)
  DEBUG((EFI_D_ERROR, "[RAS]  RAS_PARITY_ERROR_STATUS_UCP_GRP2 = 0x%08x\n", RasValue));
      RasSmnRead(0, 0x13B2006c, &RasValue);
  if (RasValue)
  DEBUG((EFI_D_ERROR, "[RAS]  RAS_PARITY_ERROR_STATUS_UCP_GRP3 = 0x%08x\n", RasValue));
      RasSmnRead(0, 0x13B20070, &RasValue);
  if (RasValue)
  DEBUG((EFI_D_ERROR, "[RAS]  RAS_PARITY_ERROR_STATUS_UCP_GRP4 = 0x%08x\n", RasValue));
      RasSmnRead(0, 0x13B20074, &RasValue);
  if (RasValue)
  DEBUG((EFI_D_ERROR, "[RAS]  RAS_PARITY_ERROR_STATUS_UCP_GRP5 = 0x%08x\n", RasValue));
        RasSmnRead(0, 0x13B20410, &RasValue);
if (RasValue)
  DEBUG((EFI_D_ERROR, "[RAS]  RAS_INTERNAL_POISION_STATUS = 0x%08x\n", RasValue));
          RasSmnRead(0, 0x13B20420, &RasValue);
if (RasValue)
  DEBUG((EFI_D_ERROR, "[RAS]  RAS_EGRESS_POISION_STATUS_Lo = 0x%08x\n", RasValue));
  RasSmnRead(0, 0x13B20424, &RasValue);
if (RasValue)
  DEBUG((EFI_D_ERROR, "[RAS]  RAS_EGRESS_POISION_STATUS_Hi = 0x%08x\n", RasValue));
...
//[-start-240426-IB18160212-modify-]//
  ...
  Status = gBS->AllocatePool (EfiACPIMemoryNVS, BertTable->BootErrorRegionLength + RecordLen, (VOID**)&NewBertErrRgnTable);
  if (EFI_ERROR (Status))  {
    return Status;
  }
//[-end-240426-IB18160212-modify-]//
...
//[-start-240426-IB18160212-modify-]//
  ...
  Status = gBS->AllocatePool (EfiACPIMemoryNVS, OldHestTable->Header.Length + RecordLen, (VOID**)&NewHestTable);
//[-end-240426-IB18160212-modify-]//
  ...
  )
{
  ...
  for (i = 0; i < BankNum; i++) {
    ...
    if (McaStatusMsr.Field.Val) {
	  ...
	  DEBUG((EFI_D_ERROR, "[RAS] McaStatusMsr reg[%d] = 0x%lx\n", i, RasMcaErrorInfo->McaBankErrorInfo[i].McaStatusMsr.Value));
	  DEBUG((EFI_D_ERROR, "[RAS] McaAddrMsr   reg[%d] = 0x%lx\n", i, RasMcaErrorInfo->McaBankErrorInfo[i].McaAddrMsr.Value));
	  DEBUG((EFI_D_ERROR, "[RAS] McaConfigMsr reg[%d] = 0x%lx\n", i, RasMcaErrorInfo->McaBankErrorInfo[i].McaConfigMsr.Value));
      DEBUG((EFI_D_ERROR, "[RAS] McaIpidMsr   reg[%d] = 0x%lx\n", i, RasMcaErrorInfo->McaBankErrorInfo[i].McaIpidMsr.Value));
	  DEBUG((EFI_D_ERROR, "[RAS] McaSyndMsr   reg[%d] = 0x%lx\n", i, RasMcaErrorInfo->McaBankErrorInfo[i].McaSyndMsr.Value));
      DEBUG((EFI_D_ERROR, "[RAS] McaMisc0Msr  reg[%d] = 0x%lx\n", i, RasMcaErrorInfo->McaBankErrorInfo[i].McaMisc0Msr.Value));
      DEBUG((EFI_D_ERROR, "[RAS] McaMisc1Msr  reg[%d] = 0x%lx\n", i, RasMcaErrorInfo->McaBankErrorInfo[i].McaMisc1Msr.Value));
      DEBUG((EFI_D_ERROR, "[RAS] McaDeStatMsr reg[%d] = 0x%lx\n", i, RasMcaErrorInfo->McaBankErrorInfo[i].McaDeStatMsr.Value));  
	  DEBUG((EFI_D_ERROR, "[RAS] McaDeAddrMsr reg[%d] = 0x%lx\n", i, RasMcaErrorInfo->McaBankErrorInfo[i].McaDeAddrMsr.Value));
      }
    }
}
//[-end-240426-IB18160212-modify-]//
