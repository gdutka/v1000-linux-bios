...
//[-start-240426-IB18160212-modify-]//
  ...
  Status = gBS->LocateProtocol (&gAmdCoreTopologyServicesProtocolGuid, NULL, (VOID**)&CoreTopology);
  ASSERT (!EFI_ERROR (Status));
...
//[-start-240426-IB18160212-modify-]//
  ...
  CalledStatus = gBS->LocateProtocol (&gAmdCoreTopologyServicesProtocolGuid, NULL, (VOID**)&CoreTopologyServices);
  ASSERT (!EFI_ERROR (CalledStatus));
//[end-240426-IB18160212-modify]//
        ...
        for (DieLoop = 0; DieLoop < NumberOfDies; DieLoop++) {
          if (FabricNuma->DomainXlat (FabricNuma, SocketLoop, DieLoop, &Domain) ==  EFI_SUCCESS) {
            if ((Domain != PreDomain) || (Domain == 0)) {
              ...
              CratHsaEntry->ProcessorIdLow   = CcxZenRvCalcLocalApic (SocketLoop, DieLoop, 0, 0, 0);
