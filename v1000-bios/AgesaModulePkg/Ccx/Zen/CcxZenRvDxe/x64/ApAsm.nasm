...
ASM_PFX(ApAsmCode):
  ...
  mov rsi, ASM_PFX(ApStackBasePtr)
  mov rsi, [rsi]
  add rax, rsi
  mov rsp, rax
  ...
  mov rsi, ASM_PFX(BspMsrLocation)
  mov esi, [rsi]
MsrStart:
  mov ecx, [esi]
  cmp ecx, 0FFFFFFFFh
...
Tom2Disabled:
  ...
  mov rsi, ASM_PFX(AllowToLaunchNextThreadLocation)
  mov esi, [rsi]
  lock inc WORD [esi]
  ...
  ; Set up resident GDT
  mov rsi, ASM_PFX(ApGdtDescriptor)
  lgdt [rsi]
  mov rbx, NewGdtAddress
  mov rsi, NewGdtOffset
  mov [rsi], ebx
  mov rsi, NewGdtOffset
  jmp far [rsi]
NewGdtAddress:
  ...
  mov rsi, ASM_PFX(ApSyncLocation)
  mov esi, [rsi]
  lock inc WORD [esi]
