...
#define DxioInputParsetGetNextDescriptor(Descriptor) (Descriptor != NULL ? ((((Descriptor->Flags & DESCRIPTOR_TERMINATE_LIST) != 0) ? NULL : (Descriptor+1))) : NULL)
  ...
  )
{
  ...
  Status = (*PeiServices)->LocatePpi ((CONST EFI_PEI_SERVICES**)PeiServices, &gAmdMemoryInfoHobPpiGuid, 0, NULL, (VOID**)&AmdMemoryInfoHob);
  if (!EFI_ERROR (Status)) {
    IDS_HDT_CONSOLE (GNB_TRACE, "AmdMemoryInfoHobPpiGuid is installed\n");
    Status = (*PeiServices)->LocatePpi ((CONST EFI_PEI_SERVICES**)PeiServices, &gAmdNbioSmuServicesPpiGuid, 0, NULL, (VOID**)&SmuServicesPpi);
    if (!EFI_ERROR (Status)) {
      ...
      if (!EFI_ERROR (Status)) {
        ...
        while (GnbHandle != NULL) {
          ...
          if ((LogicalId.Family & AMD_FAMILY_17_RV) && (LogicalId.Revision & AMD_REV_F17_RV_A0) == TRUE) {
            KpnpResetWorkaround ((CONST EFI_PEI_SERVICES**)PeiServices, GnbHandle);
            A0ParallelTrainingWorkaround ((CONST EFI_PEI_SERVICES**)PeiServices, GnbHandle, PcieTopologyData);
          }
        ...
        Status = DxioEarlyInitV1 ((CONST EFI_PEI_SERVICES**)PeiServices, SmuServicesPpi, Pcie, PcieTopologyData);
        ...
        Status = (**PeiServices).InstallPpi ((CONST EFI_PEI_SERVICES**)PeiServices, &mNbioPcieTrainingDonePpiList);
      }
    }
  }
