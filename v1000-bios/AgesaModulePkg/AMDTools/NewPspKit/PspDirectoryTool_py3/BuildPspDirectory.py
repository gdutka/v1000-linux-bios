...
ProgramTable = {
  'ZP': {"AndMask":0xFFFF0000,"PSPID": 0xBC090000, "BIOSDIR_OFFSET": 6},
  'RV': {"AndMask":0xFFFF0000,"PSPID": 0xBC0A0000, "BIOSDIR_OFFSET": 7},
  'SSP': {"AndMask":0xFFFF0000,"PSPID": 0xBC0B0000, "BIOSDIR_OFFSET": 8},
  'ARD': {"AndMask":0xFFFF0000,"PSPID": 0xBC0C0000, "BIOSDIR_OFFSET": 7},
}
...
def prettify(elem):
    ...
    """
    rough_string = tostring(elem, 'us-ascii')
...
# Validate Program string
  if (args.subcommand == 'dp'):
    ...
    if (ValidProgram == False):
      log.info ("[Error] Not a valid program string, only %s are supported", list(ProgramTable.keys()))
      FatalErrorExit ()
...
def ParsePspDirBin (FileHandle, BinarySize, PspDirOffSetInBin, Level, PspDirectoryInfos):
  ...
  for i in range (NumEntries):
    ...
    if (Size != 0xFFFFFFFF):
      ...
      EntryInfo = {'Type16':Type16,'Type':Type, 'RomId':RomId,'Size':Size,'RTOffset':Location, 'Description':getEntryName(Type & 0xFF), 'Address':getOffsetInBin(BinarySize, Location)}
...
def GetPspDirectory (BinaryFile):
  ...
  # Loop the RomSigAddrTable to search the ROMSIG, exit the loop if 1st found
  for RomSigPhyAddr in RomSigAddrTable:
    RomSigAddr = getRelativeAddress (BinarySize, RomSigPhyAddr)
    if (RomSigAddr):
      log.info ("Check ROMSIG @ 0x%X" ,RomSigAddr)
      FileHandle.seek (RomSigAddr)
      ...
      if (OemSigArray[ROMSIG_SIG_OFFSET] == 0x55aa55aa):
        ...
        if (Signature == PSP_DIR_SIGNATURE):
          ...
          if (BiosDirRTOffset != 0):
            log.info ("BIOS Directory Offset found at ROMSIG[%x]0x%X" ,ProgramTable[ProgramStr]["BIOSDIR_OFFSET"], BiosDirRTOffset)
            #convert to raw binary offset
...
def KvmInfo (bios, entry):
  ...
  verOffset = 0x60
  KvmInfo = {}
  KvmHeader = bios [entry['Address']:entry['Address']+KvmHeaderSize]
...
def OutPutPspDirInfoXml (BinaryInfo, PspDirectoryInfos, BiosDirectoryInfos, xmlFileName):
  ...
  print(prettify(root), file=FileHandle)
