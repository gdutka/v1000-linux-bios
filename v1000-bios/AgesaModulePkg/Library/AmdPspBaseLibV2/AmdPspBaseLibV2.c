  ...
  )
{
  ...
  BOOLEAN             TwoSlotAbRecovery = FALSE;

  ASSERT (PspL1Dir != NULL);
  TwoSlotAbRecovery = IsTwoSlotAbRecovery (PspL1Dir);
  ...
  TempSlot->SlotCount = PspL1Dir->Header.TotalEntries;
  TempSlot->BootableSlotCount = 0;
  TempSlot->UnbootableSlotCount = 0;

  TempSlot->ActiveSlotIndex = 0xFF;
  TempSlot->ActiveSlotHeader.Priority = Unbootable;
  TempSlot->ActiveSlotHeader.UpdateRetries = 0;
  TempSlot->ActiveSlotHeader.GlitchRetries = 0;
  TempSlot->ActiveSlotHeader.ImageSlotAddr = 0;

  TempSlot->AlternativeSlotIndex = 0xFF;
  TempSlot->AlternativeSlotHeader.Priority = Unbootable;
  TempSlot->AlternativeSlotHeader.UpdateRetries = 0;
  TempSlot->AlternativeSlotHeader.GlitchRetries = 0;
  TempSlot->AlternativeSlotHeader.ImageSlotAddr = 0;
  ...
  for (Index = 0; Index < PspL1Dir->Header.TotalEntries; Index++) {
    if ((PspL1Dir->PspEntry[Index].Type.Field.Type == PSP_REGION_A_DIR)
      ...
      if (FchSpiRomReadEx ((UINT32)(TempImageSlotAddr & 0xFFFFFFFF), (UINT8 *)&ImageSlotEntry, sizeof (IMAGE_SLOT_HEADER)) != TRUE) {
        ASSERT (FALSE);
        return FALSE;
      }

      //find image slots with top 2 priorities
      if (ImageSlotEntry.Priority > HighestPriority) {
        break;
      } else {
        if (ImageSlotEntry.Priority == Unbootable) {
        ...
        //VN considers unbootable parition as alternative slot but MR doesn't
        if ((ImageSlotEntry.Priority != Unbootable) || TwoSlotAbRecovery) {
          //if we found image slot with higher priority, update active image slot
          if (ImageSlotEntry.Priority > TempSlot->ActiveSlotHeader.Priority) {
            //save TempSlot->ActiveSlot to AlternativeSlot
            if (TempSlot->ActiveSlotHeader.Priority != Unbootable) {
              TempSlot->AlternativeSlotIndex = TempSlot->ActiveSlotIndex;
              TempSlot->AlternativeSlotHeader.Priority = TempSlot->ActiveSlotHeader.Priority;
              TempSlot->AlternativeSlotHeader.UpdateRetries = TempSlot->ActiveSlotHeader.UpdateRetries;
              TempSlot->AlternativeSlotHeader.GlitchRetries = TempSlot->ActiveSlotHeader.GlitchRetries;
              TempSlot->AlternativeSlotHeader.ImageSlotAddr = TempSlot->ActiveSlotHeader.ImageSlotAddr;
            }
            //update ActiveSlot
            TempSlot->ActiveSlotIndex = Index;
            TempSlot->ActiveSlotHeader.Priority = ImageSlotEntry.Priority;
            TempSlot->ActiveSlotHeader.UpdateRetries = ImageSlotEntry.UpdateRetries;
            TempSlot->ActiveSlotHeader.GlitchRetries = ImageSlotEntry.GlitchRetries;
            TempSlot->ActiveSlotHeader.ImageSlotAddr = ImageSlotEntry.ImageSlotAddr;
          } else if ((ImageSlotEntry.Priority > TempSlot->AlternativeSlotHeader.Priority)
                    && (TempSlot->AlternativeSlotIndex != TempSlot->ActiveSlotIndex)) {
              TempSlot->AlternativeSlotIndex = Index;
              TempSlot->AlternativeSlotHeader.Priority = ImageSlotEntry.Priority;
              TempSlot->AlternativeSlotHeader.UpdateRetries = ImageSlotEntry.UpdateRetries;
              TempSlot->AlternativeSlotHeader.GlitchRetries = ImageSlotEntry.GlitchRetries;
              TempSlot->AlternativeSlotHeader.ImageSlotAddr = ImageSlotEntry.ImageSlotAddr;
          }
        }
      }
    } else { //break the search if we found entries other than PSP_REGION_A_DIR/PSP_REGION_B_DIR
      break;
    }
  }
  ...
  if ((!IS_VALID_ADDR32(TempSlot->ActiveSlotHeader.ImageSlotAddr)) ||
      (!ALIGN_4K_CHECK (TempSlot->ActiveSlotHeader.ImageSlotAddr))) { // no valid Active Image Slot address found
    return FALSE;
  }
...
/**
  This function is to get the PSP entry information for given PSP entry type on a give image slot.
  @param[in]   ImageSlotAddr  image slot address, usually point to PSP L2 directory
  @param[in]   EntryType      value of given PSP entry type
  @param[out]  EntryAddress   pointer to PSP entry address
  @param[out]  EntrySize      pointer to PSP entry size
  ...
  )
{
  ...
  if (IsMultiSlotLayout (PspDir)) {
    if (GetLevel2PSPEntryInfo (EntryType, ImageSlotAddr, EntryAddress, EntrySize) == TRUE) {
      return (TRUE);
    }
  }
  return (FALSE);
