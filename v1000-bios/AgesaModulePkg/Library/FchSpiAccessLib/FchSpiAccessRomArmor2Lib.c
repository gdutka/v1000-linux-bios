  ...
  )
{
  ...
  if (IS_SPI_ROM2_OFFSET (Location) && ((Location + Length) <= FCH_ROM_SIZE_16M)) {
    FlashAddress = FCH_ROM_START_ADDRESS_2 + Location;
  } else if (IS_SPI_ROM3_OFFSET (Location) && ((Location + Length) <= FCH_ROM_SIZE_64M)) {
    FlashAddress = FCH_ROM_START_ADDRESS_3 + Location;
  } else if (IS_IN_SPI_ROM2_WINDOW (Location) || IS_IN_SPI_ROM3_WINDOW (Location)) {
    FlashAddress = Location;
  } else {
    return FALSE;
  }
  ...
  if (Status == EFI_NOT_FOUND) {
    CopyMem (Data, (VOID *)FlashAddress, NumByte);
  }
