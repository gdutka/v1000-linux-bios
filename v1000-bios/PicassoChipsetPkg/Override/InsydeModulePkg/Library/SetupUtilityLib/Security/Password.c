    ...
    if (SUCInfo->HddPasswordScuData[Index].Flag == CHANGE_PASSWORD) {
      ...
      if (Status == EFI_SUCCESS) {
        ...
        if (SUCInfo->HddPasswordScuData[Index].HddInfo->StorageTcgSecuritySupported &&
            SUCInfo->HddPasswordScuData[Index].DisableAllType == NO_ACCESS_PSW) {
          //
          // Single password scenario
          // USER password is the same as MASTER password. Record as SID/Admin1
          //
  ...
  )
{
  ...
  // TCG PSID
  //
  mStoredPsid = (TCG_PSID *)AllocateZeroPool (NumOfHdd * sizeof (TCG_PSID));
  if (mStoredPsid == NULL) {
    return EFI_OUT_OF_RESOURCES;
  }
  ...
  )
{
  ...
  if (HddPasswordScuData[HddIndex].Flag == SECURITY_LOCK) {
    ...
    if (mTakeOwnershipByUserPassword || (HddPasswordScuData[HddIndex].Flag != NO_ACCESS_PASSWORD)) {
      HiiCreatePasswordOpCodeEx (
        StartUserPasswordOpCodeHandle,
