  ...
  )
{
  ...
  if (TcgPpData->PPRequest >= TCG_PHYSICAL_PRESENCE_VENDOR_SPECIFIC_OPERATION) {
    ...
    if (!RequestConfirmed) {
      ...
      //
      DEBUG ((DEBUG_INFO, "OemKernelServices Call: OemSvcTpmUserConfirmDialog \n"));
      Status = OemSvcTpmUserConfirmDialog (TcgPpData->PPRequest, TPM_DEVICE_1_2, &RequestConfirmed);
      DEBUG ((DEBUG_INFO, "OemKernelServices OemSvcTpmUserConfirmDialog Status: %r\n", Status));
      if (EFI_ERROR (Status)) {
  ...
  )
{
  ...
  BOOLEAN                           CmdEnable;
//[-start-150610-IB06170028-add]//
  EFI_TCG_PROTOCOL                  *TcgProtocol;
//[-end-150610-IB06170028-add]//
  ...
  //
//[-start-150610-IB06170028-add]//
  Status = TpmPhysicalPresence (TPM_PHYSICAL_PRESENCE_LIFETIME_LOCK);
//[-end-150610-IB06170028-add]//
...
//[-start-150610-IB06170028-add]//
  Status = gBS->LocateProtocol (&gEfiTcgProtocolGuid, NULL, (VOID **)&TcgProtocol);
  if (!EFI_ERROR (Status)) {
    Status = mTpmPhysicalPresenceProtocol.TpmNvLock (TcgProtocol);
  }
