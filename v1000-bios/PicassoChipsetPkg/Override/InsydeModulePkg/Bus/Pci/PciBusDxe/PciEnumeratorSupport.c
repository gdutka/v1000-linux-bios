  ...
  )
{
  ...
  // OemServices
  //
  DEBUG ((DEBUG_INFO, "OemKernelServices Call: OemSvcInstallPciSkipTable \n"));
  Status = OemSvcInstallPciSkipTable (
	     &PciSkipTableCount,
             &TempPciSkipTable
	     );
  DEBUG ((DEBUG_INFO, "OemKernelServices OemSvcInstallPciSkipTable Status: %r\n", Status));
  ...
  for (TableIndex = 0; PciSkipTable[TableIndex].VendorId != 0xffff; TableIndex++) {
    ...
    // (OemServices)
    //
    DEBUG ((DEBUG_INFO, "OemKernelServices Call: OemSvcUnSkipPciDevice \n"));
    ...
    DEBUG ((DEBUG_INFO, "OemKernelServices OemSvcUnSkipPciDevice Status: %r\n", Status));
  ...
  )
{
  ...
  if (FeaturePcdGet (PcdH2OBdsCpPciEnumUpdateDevInfoSupported)) {
    ...
    H2O_PCI_IO_DEVICE                                  H2OPciIoDevie;
    ...
    PciEnumUpdateDevInfoData.PciDevice            = &H2OPciIoDevie;
    SetH2OPciIoDeviceInfo (&H2OPciIoDevie, PciIoDevice);
    ...
    if (PciEnumUpdateDevInfoData.Status == H2O_CP_TASK_UPDATE) {
      SetPciIoDeviceInfo (PciIoDevice, &H2OPciIoDevie);
    }
  }
  //
