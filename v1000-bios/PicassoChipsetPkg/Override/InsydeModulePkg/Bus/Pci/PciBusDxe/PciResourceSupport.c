...
/**
  ...
  @param Bridge      PCI resoure node for given bridge device.
  ...
  )
{
  ...
  PCI_AVAILABLE_SPACE_DESCRIPTOR   *TotalUsedSpace = NULL;
  PCI_AVAILABLE_SPACE_DESCRIPTOR   *AvailiableSpace = NULL;
  PCI_AVAILABLE_SPACE_DESCRIPTOR   *AvailiableSpacePart1 = NULL;
  PCI_AVAILABLE_SPACE_DESCRIPTOR   *AvailiableSpacePart2 = NULL;
  ...
  //
  TotalUsedSpace = AllocateZeroPool (sizeof (PCI_AVAILABLE_SPACE_DESCRIPTOR));
  ...
  while (CurrentLink != &Bridge->ChildList) {
    ...
    if (!IsListEmpty (&TotalUsedSpace->Link)) {
      ...
      while (AvailiableSpaceLink != &TotalUsedSpace->Link) {
        ...
        AvailiableSpace = AVAILABLE_SPACE_FROM_LINK (AvailiableSpaceLink);
        ...
        if (NodeEndAddress <= AvailiableSpace->EndAddress) {
          ...
          if ((Offset != 0) && (NodeEndAddress < AvailiableSpace->EndAddress)) {
            AvailiableSpacePart1 = AllocateZeroPool (sizeof (PCI_AVAILABLE_SPACE_DESCRIPTOR));
            AvailiableSpacePart2 = AllocateZeroPool (sizeof (PCI_AVAILABLE_SPACE_DESCRIPTOR));
    ...
    if (FoundAvailableSpace == FALSE) {
      ...
      if (NodeBaseAddress > TotalUsedSpace->EndAddress) {
        AvailiableSpace = AllocateZeroPool (sizeof (PCI_AVAILABLE_SPACE_DESCRIPTOR));
  ...
  )
{
  ...
  if (FeaturePcdGet (PcdPciDegradeResourceForOptionRom) && BootType != EFI_BOOT_TYPE) {
    //
...
//
// redirect eMMC device BAR to eMMC host controller,
// eMMC host controller address can be found at ppr,
// appendix Memory map Main-Memory EMMCHC
//
  if (Node->PciDev->BusNumber      == 0x0  &&
      Node->PciDev->DeviceNumber   == 0x13 &&
      Node->PciDev->FunctionNumber == 0x0) {
  Address = 0xFEDD5000;
  }
  ...
  )
{
  ...
  while (Parent != NULL) {
    ...
    if (Enable) {
      ...
      ProgramPpbAperture (OptionRomBase, &Node);
      ...
      ProgramPpbAperture (Parent->PciBar[Node.Bar].BaseAddress, &Node);
