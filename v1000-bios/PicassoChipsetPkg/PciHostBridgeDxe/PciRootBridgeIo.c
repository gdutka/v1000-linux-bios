...
//[-start-191129-IB06070066-modify]//
    if (PciRbAddr->ExtendedRegister != 0) {
      Address = PciRbAddr->ExtendedRegister;
      Limit = MAX_PCI_EXP_REG_ADDRESS;
    } else {
      Address = PciRbAddr->Register;
      Limit = MAX_PCI_REG_ADDRESS;
    }
    Base = 0;
//[-end-191129-IB06070066-modify]//
  }
...
//
// redirect PCI 00:13.00 to eMMC controller
// eMMC configuration address can be find from ppr appendix
// Memory Map Main-Memory EMMCCFG
//
  EmmcBaseAddr  = 0x00FEDD5800;
  if (PciRbAddr->Bus      == 0x00 &&
      PciRbAddr->Device   == 0x13 &&
      PciRbAddr->Function == 0x00) {
    Reg = PciRbAddr->Register;
    PcieRegAddr = EmmcBaseAddr + Reg;
  }
  ...
  for (Uint8Buffer = Buffer; Count > 0; PcieRegAddr += InStride, Uint8Buffer += OutStride, Count--) {
    if (PciRbAddr->Bus      == 0x00 &&
      ...
      if (Write) {
        switch (OperationWidth) {
          case EfiPciWidthUint8:
            MmioWrite8 ((UINTN)PcieRegAddr, *Uint8Buffer);
            break;
          case EfiPciWidthUint16:
            MmioWrite16 ((UINTN)PcieRegAddr, *((UINT16 *)Uint8Buffer));
            break;
          case EfiPciWidthUint32:
            MmioWrite32 ((UINTN)PcieRegAddr, *((UINT32 *)Uint8Buffer));
            break;
          case EfiPciWidthUint64:
            MmioWrite64 ((UINTN)PcieRegAddr, *((UINT64 *)Uint8Buffer));
            break;
          default:
            //
            // The RootBridgeIoCheckParameter call above will ensure that this
        ...
        switch (OperationWidth) {
          case EfiPciWidthUint8:
            *Uint8Buffer = MmioRead8 ((UINTN)PcieRegAddr);
            break;
          case EfiPciWidthUint16:
            *((UINT16 *)Uint8Buffer) = MmioRead16 ((UINTN)PcieRegAddr);
            break;
          case EfiPciWidthUint32:
            *((UINT32 *)Uint8Buffer) = MmioRead32 ((UINTN)PcieRegAddr);
            break;
          case EfiPciWidthUint64:
            *((UINT64 *)Uint8Buffer) = MmioRead64 ((UINTN)PcieRegAddr);
            break;
          default:
            //
            // The RootBridgeIoCheckParameter call above will ensure that this
