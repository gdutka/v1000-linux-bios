...
//1CCF76B2-E536-4069-98C4-F129FEEAB610
#define AMD_PEI_RV2_VBIOS_FFS_GUID\
  { 0x1CCF76B2, 0xE536, 0x4069, {0x98, 0xC4, 0xF1, 0x29, 0xFE, 0xEA, 0xB6, 0x10} }
  ...
  )
{
  ...
  Framebuffer = (UINT8 *)((UINTN)Mode.FrameBufferBase);
  Instance = 0;
  while (TRUE) {
    ...
    if (!EFI_ERROR (Status)) {
      Status = PeiServicesFfsFindSectionData (EFI_SECTION_RAW, FileHandle, (VOID **)&LogoImage);
      if (EFI_ERROR (Status)) {
...
PreaprePeiGopPolicyData (VOID)
{
  ...
  EFI_GUID                Rv2VbiosGuid = AMD_PEI_RV2_VBIOS_FFS_GUID;
  UINT8                   NextBridgeNum;
  ...
  while (TRUE) {
    ...
    if (CheckRvOpnB4()) {
      DEBUG ((EFI_D_ERROR, "Load RV1/PCO VBIOS!\n"));
      Status = PeiServicesFfsFindFileByName (&VbiosGuid, VolumeHandle, &FileHandle);
    } else {
      DEBUG ((EFI_D_ERROR, "Load RV2 VBIOS!\n"));
      Status = PeiServicesFfsFindFileByName (&Rv2VbiosGuid, VolumeHandle, &FileHandle);
    }
    if (!EFI_ERROR (Status)) {
  ...
  )
{
  ...
  if (!EFI_ERROR(Status)) {
    DrawAmdLogo((CONST EFI_PEI_SERVICES**)PeiServices);
  }
  return Status;
}
