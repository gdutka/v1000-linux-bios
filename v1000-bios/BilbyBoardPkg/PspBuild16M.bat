...
set BIOS_FV_PATH=%WORKSPACE%\Build\%PROJECT_PKG%\%TARGET%_%TOOL_CHAIN_TAG%\FV
  ...
  set BIOS_IMAGE_CONFIG_FILE=%PSP_CONFIG_FILE_PATH%\BIOSImageDirectory16M.xml
if "%KEY_MODE%"=="NOSIGN" (
  REM remove the line that conatins RTMSignature string for NOSIGN mode
  @findstr /v "RTMSignature" %BIOS_IMAGE_CONFIG_FILE% > %PSP_CONFIG_FILE_PATH%\BIOSImageDirectory16M-NOSIGN.xml
  set BIOS_IMAGE_CONFIG_FILE=%PSP_CONFIG_FILE_PATH%\BIOSImageDirectory16M-NOSIGN.xml
)
