  ...
  )
{
  ...
  //RPMC Control
  if (Setup_Config->CbsCmnSocRpmcControl != 2) {
    if (ApcbParaAttrib != NULL) {
      ApcbParaAttrib->TimePoint = APCB_TIME_POINT_ANY;
      ApcbParaAttrib->Token = APCB_TOKEN_CBS_CMN_SOC_RPMC_CONTROL;
  ...
  if (ApcbParaAttrib != NULL) {
    ApcbParaAttrib->TimePoint = APCB_TIME_POINT_ANY;
    ApcbParaAttrib->Token = APCB_TOKEN_CBS_CMN_PERSISTENT_LOW;
  ...
  if (ApcbParaAttrib != NULL) {
    ApcbParaAttrib->TimePoint = APCB_TIME_POINT_ANY;
    ApcbParaAttrib->Token = APCB_TOKEN_CBS_CMN_PERSISTENT_HIGH;
    ApcbParaAttrib->Size = (sizeof (UINT32) -1);
    ApcbParaAttrib++;
  }
  _Size += sizeof (APCB_PARAM_ATTRIBUTE);
  ...
  if (ApcbParaAttrib != NULL) {
    ApcbParaAttrib->TimePoint = APCB_TIME_POINT_ANY;
    ApcbParaAttrib->Token = APCB_TOKEN_CBS_CMN_PERSISTENT_SIZE_LOW;
    ApcbParaAttrib->Size = (sizeof (UINT32) -1);
    ApcbParaAttrib++;
  }
  _Size += sizeof (APCB_PARAM_ATTRIBUTE);
  ...
  if (ApcbParaAttrib != NULL) {
    ApcbParaAttrib->TimePoint = APCB_TIME_POINT_ANY;
    ApcbParaAttrib->Token = APCB_TOKEN_CBS_CMN_PERSISTENT_SIZE_HIGH;
    ApcbParaAttrib->Size = (sizeof (UINT32) -1);
    ApcbParaAttrib++;
  }
  _Size += sizeof (APCB_PARAM_ATTRIBUTE);
  ...
  if (ApcbParaAttrib != NULL) {
    ApcbParaAttrib->TimePoint = APCB_TIME_POINT_ANY;
    ApcbParaAttrib->Token = APCB_TOKEN_CBS_CMN_SPD_SMBUS_VOLTAGE;
    ApcbParaAttrib->Size = (sizeof (UINT8) -1);
    ApcbParaAttrib++;
  }
  _Size += sizeof (APCB_PARAM_ATTRIBUTE);
  ...
  if (ApcbParaAttrib != NULL) {
    ApcbParaAttrib->TimePoint = APCB_TIME_POINT_ANY;
    ApcbParaAttrib->Token = APCB_TOKEN_CBS_CMN_MEM_DIS_MEM_ERR_INJ;
    ApcbParaAttrib->Size = (sizeof (UINT8) -1);
    ApcbParaAttrib++;
  }
  _Size += sizeof (APCB_PARAM_ATTRIBUTE);
  ...
  //UMA Above 4G
  if (Setup_Config->CbsDfCmnUmaAbove4G != 0xFF) {
    if (ApcbParaAttrib != NULL) {
      ApcbParaAttrib->TimePoint = APCB_TIME_POINT_ANY;
      ApcbParaAttrib->Token = APCB_TOKEN_CBS_DF_CMN_UMA_ABOVE4_G;
      ApcbParaAttrib->Size = (sizeof (UINT8) -1);
      ApcbParaAttrib++;
    }
    _Size += sizeof (APCB_PARAM_ATTRIBUTE);
  ...
  )
{
  ...
  if (Setup_Config->CbsCmnSocRpmcControl != 2) {
    if (ApcbParaValue != NULL) {
      * ((UINT8 *) ApcbParaValue) = Setup_Config->CbsCmnSocRpmcControl;
  ...
  //Persistent memory Base Low
  if (ApcbParaValue != NULL) {
    * ((UINT32 *) ApcbParaValue) = Setup_Config->CbsCmnPersistentLow;
  ...
  //Persistent memory Base High
  if (ApcbParaValue != NULL) {
    * ((UINT32 *) ApcbParaValue) = Setup_Config->CbsCmnPersistentHigh;
    ApcbParaValue += sizeof (UINT32);
  }
  _Size += sizeof (UINT32);
  ...
  //Persistent memory reserved size Low
  if (ApcbParaValue != NULL) {
    * ((UINT32 *) ApcbParaValue) = Setup_Config->CbsCmnPersistentSizeLow;
    ApcbParaValue += sizeof (UINT32);
  }
  _Size += sizeof (UINT32);
  ...
  //Persistent memory reserved size High
  if (ApcbParaValue != NULL) {
    * ((UINT32 *) ApcbParaValue) = Setup_Config->CbsCmnPersistentSizeHigh;
    ApcbParaValue += sizeof (UINT32);
  }
  _Size += sizeof (UINT32);
  ...
  //SMBUS SPD Voltage
  if (ApcbParaValue != NULL) {
    * ((UINT8 *) ApcbParaValue) = Setup_Config->CbsCmnSpdSmbusVoltage;
    ApcbParaValue += sizeof (UINT8);
  }
  _Size += sizeof (UINT8);
  ...
  //Disable Memory Error Injection
  if (ApcbParaValue != NULL) {
    * ((UINT8 *) ApcbParaValue) = Setup_Config->CbsCmnMemDisMemErrInj;
    ApcbParaValue += sizeof (UINT8);
  }
  _Size += sizeof (UINT8);
  ...
  if (Setup_Config->CbsDfCmnUmaAbove4G != 0xFF) {
    if (ApcbParaValue != NULL) {
      * ((UINT8 *) ApcbParaValue) = Setup_Config->CbsDfCmnUmaAbove4G;
      ApcbParaValue += sizeof (UINT8);
    }
    _Size += sizeof (UINT8);
  }
