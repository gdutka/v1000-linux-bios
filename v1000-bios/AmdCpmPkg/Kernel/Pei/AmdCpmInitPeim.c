  ...
  )
{
  ...
  Status = (**PeiServices).NotifyPpi ((CONST EFI_PEI_SERVICES**)PeiServices, &mCpmInitPeimNotify);
  if (EFI_ERROR (Status)) {
    DEBUG((DEBUG_INFO, "KER-PEI-%a-End-1-Status=%r\n", __FUNCTION__, Status));
    return Status;
  }
  ...
  )
{
  ...
  if (CpmBootMode != CPM_BOOT_MODE_S3) {
    ...
    Status = (**PeiServices).NotifyPpi ((CONST EFI_PEI_SERVICES**)PeiServices, &mNotifyListS3Restore);
  }
  ...
  Status = (**PeiServices).NotifyPpi ((CONST EFI_PEI_SERVICES**)PeiServices, &mCpmPtInitPeimNotify);
