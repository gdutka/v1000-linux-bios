  ...
  )
{
  ...
  CommandValue = PciRead16 (AddressValue);
   ...
   //Check for root bridge presence
  if(CommandValue == 0xFFFF)
  return;

  //Check for first 3-bits of command register for IO/Memory/Bus Master enable
  if( (CommandValue & 0x6) != 0x6)
  {
    CommandValue = (CommandValue | 0x6);
  }
  ...
  )
{
  ...
  Status = gBS->LocateProtocol (&gxGbEI2cMasterProtocolGuid, NULL, (VOID**)&XgbEI2cMaster);
  if (EFI_ERROR (Status)) {
