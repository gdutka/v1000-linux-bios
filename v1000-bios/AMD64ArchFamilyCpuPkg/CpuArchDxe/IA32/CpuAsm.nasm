...
extern _mErrorCodeFlag                  ; Error code flags for exceptions
...
global ASM_PFX(_InitializeIdt)
ASM_PFX(_InitializeIdt):
  push    ebp               ; C prolog
  mov     ebp, esp
  push    edi
...
VectorNumber:   
   ...
   jmp  dword [CommonInterruptEntry];
TemplateEnd:
...
global ASM_PFX(_GetTemplateAddressMap)
ASM_PFX(_GetTemplateAddressMap):
  push    ebp                 ; C prolog
  mov     ebp, esp
  pushad

  mov ebx, dword [ebp+08h]
  mov dword [ebx,    TemplateStart]
  mov dword [ebx+4h, TemplateEnd - TemplateStart]
...
global ASM_PFX(_InitializeGdt)
ASM_PFX(_InitializeGdt):
  push    ebp                 ; C prolog
  mov     ebp, esp
  pushad
