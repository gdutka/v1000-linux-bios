...
SMM_CPU_PRIVATE_DATA  mSmmCpuPrivateData = {
  ...
  {{ 0 }},                                        // SmmReservedSmramRegion
  {
    ...
    mSmmCpuPrivateData.CpuSaveState             // SmmCoreEntryContext.CpuSaveState
  },
  NULL,                                         // SmmCoreEntry
  {
...
//  (EFI_SMM_CPU_STATE  **)mSmmCpuPrivateData.CpuSaveState
  (AMD_SMM_CPU_STATE  **)mSmmCpuPrivateData.CpuSaveState
};
...
///
#define SMM_CPU_OFFSET(Field) OFFSET_OF (AMD_SMM_CPU_STATE, Field)
...
CONST CPU_SMM_SAVE_STATE_REGISTER_RANGE mSmmCpuRegisterRanges[] = {
  SMM_REGISTER_RANGE (EFI_SMM_SAVE_STATE_REGISTER_GDTBASE, EFI_SMM_SAVE_STATE_REGISTER_LDTLIMIT),
...
CONST CPU_SMM_SAVE_STATE_LOOKUP_ENTRY mSmmCpuWidthOffset[] = {
  ...
  {4, 8, SMM_CPU_OFFSET (x86.GDTBase)   , SMM_CPU_OFFSET (x64.GDTR_Base_LoDword) , SMM_CPU_OFFSET (x64.GDTR_Base_HiDword) },  //  EFI_SMM_SAVE_STATE_REGISTER_GDTBASE  = 4
  {0, 8, 0                              , SMM_CPU_OFFSET (x64.IDTR_Base_LoDword) , SMM_CPU_OFFSET (x64.IDTR_Base_LoDword) },  //  EFI_SMM_SAVE_STATE_REGISTER_IDTBASE  = 5
  {0, 8, 0                              , SMM_CPU_OFFSET (x64.LDTR_Base_LoDword) , SMM_CPU_OFFSET (x64.LDTR_Base_LoDword) },  //  EFI_SMM_SAVE_STATE_REGISTER_LDTBASE  = 6
  {0, 2, 0                              , SMM_CPU_OFFSET (x64.GDTR_Limit)        , 0                                      },  //  EFI_SMM_SAVE_STATE_REGISTER_GDTLIMIT = 7
  {0, 2, 0                              , SMM_CPU_OFFSET (x64.IDTR_Limit)        , 0                                      },  //  EFI_SMM_SAVE_STATE_REGISTER_IDTLIMIT = 8
  {0, 4, 0                              , SMM_CPU_OFFSET (x64.LDTR_Limit)        , 0                                      },  //  EFI_SMM_SAVE_STATE_REGISTER_LDTLIMIT = 9

  {4, 2, SMM_CPU_OFFSET (x86._ES)       , SMM_CPU_OFFSET (x64._ES)               , 0                                      },  //  EFI_SMM_SAVE_STATE_REGISTER_ES       = 20
  {4, 2, SMM_CPU_OFFSET (x86._CS)       , SMM_CPU_OFFSET (x64._CS)               , 0                                      },  //  EFI_SMM_SAVE_STATE_REGISTER_CS       = 21
  {4, 2, SMM_CPU_OFFSET (x86._SS)       , SMM_CPU_OFFSET (x64._SS)               , 0                                      },  //  EFI_SMM_SAVE_STATE_REGISTER_SS       = 22
  {4, 2, SMM_CPU_OFFSET (x86._DS)       , SMM_CPU_OFFSET (x64._DS)               , 0                                      },  //  EFI_SMM_SAVE_STATE_REGISTER_DS       = 23
  {4, 2, SMM_CPU_OFFSET (x86._FS)       , SMM_CPU_OFFSET (x64._FS)               , 0                                      },  //  EFI_SMM_SAVE_STATE_REGISTER_FS       = 24
  {4, 2, SMM_CPU_OFFSET (x86._GS)       , SMM_CPU_OFFSET (x64._GS)               , 0                                      },  //  EFI_SMM_SAVE_STATE_REGISTER_GS       = 25
  {0, 2, 0                              , SMM_CPU_OFFSET (x64.LDTR_Selector)     , 0                                      },  //  EFI_SMM_SAVE_STATE_REGISTER_LDTR_SEL = 26
  {0, 2, 0                              , SMM_CPU_OFFSET (x64.TR_Selector)       , 0                                      },  //  EFI_SMM_SAVE_STATE_REGISTER_TR_SEL   = 27
  ...
  {4, 8, SMM_CPU_OFFSET (x86._ECX)      , SMM_CPU_OFFSET (x64._RCX)              , SMM_CPU_OFFSET (x64._RCX)         + 4 },   //  EFI_SMM_SAVE_STATE_REGISTER_RBX      = 39
  ...
  {0, 8, 0                              , SMM_CPU_OFFSET (x64._CR4)              , SMM_CPU_OFFSET (x64._CR4)         + 4 },   //  EFI_SMM_SAVE_STATE_REGISTER_CR4      = 54
  {0, 0, 0, 0, 0}                                                                                                                 
};
  ...
  )
{
  ...
  //
  // The offset of SMM-Revision Identifier is the same between AMD64 and AMD legacy CPU  
  // Thus, we can determine SMM-Revision Identifier from either AMD64 architecture
  // or AMD legacy architecture.
  //
  ...
  if (Register == EFI_SMM_SAVE_STATE_REGISTER_LMA) {
    ...
    if ((SmmRevId < SMM_MIN_REV_ID_x64)) {
  ...
  if (Register == EFI_SMM_SAVE_STATE_REGISTER_IO) {
    ...
    //
    // Check for the SMM_IO_TRAP register value
    //
    if (!(StateCopy->x64.SMM_IO_TRAP & 0x02)) {
      return EFI_NOT_FOUND;
    }
    ...
    //
    IoInfo->IoPort  = (UINT16) (StateCopy->x64.SMM_IO_TRAP >> 16);
    IoInfo->IoWidth = EFI_SMM_SAVE_STATE_IO_WIDTH_UINT8;
    IoInfo->IoType  = EFI_SMM_SAVE_STATE_IO_TYPE_INPUT;
    IoInfo->IoData  = (UINT64) IoRead8 ((StateCopy->x64.SMM_IO_TRAP >> 16));
  ...
  if (SmmRevId < SMM_MIN_REV_ID_x64) {
    ...
    //
    // If Width is bigger than the 32-bit mode width, then modifying the inputted width
    // to the actual 32-bit width of the register that is supported in Legacy SMM 
    // State-Save Area.
    //
    if (Width > mSmmCpuWidthOffset[RegisterIndex].Width32) {
      Width = mSmmCpuWidthOffset[RegisterIndex].Width32;         
    }
    ...
    //
    // If Width is bigger than the 64-bit mode width, then modifying the inputted width
    // to the actual 64-bit width of the register that is supported in AMD64 Architecture
    // SMM State-Save Area.
    //
    if (Width > mSmmCpuWidthOffset[RegisterIndex].Width64) {
      Width = mSmmCpuWidthOffset[RegisterIndex].Width64;       
    }
  ...
  )
{
  ...
  // Check CPU mode
  //
  if (StateCopy->x86.SMMRevId < SMM_MIN_REV_ID_x64) {
    //
    // If 32-bit mode width is zero, then the specified register can not be accessed
    //
    if (mSmmCpuWidthOffset[RegisterIndex].Width32 == 0) {
  ...
  )
{
  UINT32                            ApicId;
  UINTN                             Index;
  AMD_SMM_CPU_STATE                 *CpuState;

  ApicId = GetApicId ();

  for (Index = 0; Index < gSmmCpuPrivate->SmmCoreEntryContext.NumberOfCpus; Index++) {
    if (ApicId == gSmmCpuPrivate->ApicIds[Index]) {
      CpuState = (AMD_SMM_CPU_STATE *)(UINTN)(SMM_DEFAULT_SMBASE + SMM_CPU_STATE_OFFSET);
      CpuState->x64.SMBASE = (UINT32)gSmmCpuPrivate->SmBases[Index];

      SmmInitiFeatures (gSmmCpuPrivate->SmrrBase, gSmmCpuPrivate->SmrrSize, mIsBsp);
  ...
  )
{
  ...
  AMD_SMM_CPU_STATE                 BakBuf2;
  AMD_SMM_CPU_STATE                 *CpuStatePtr;
  UINT8                             *U8Ptr;
  UINT32                            ApicId;
  UINTN                             Index;
  ...
  CpuStatePtr = (AMD_SMM_CPU_STATE *)(UINTN)(SMM_DEFAULT_SMBASE + SMM_CPU_STATE_OFFSET);
  ...
  )
{
  ...
  // nearest power of 2.
  //
  TileSize = 2 * GetPowerOfTwo32 (sizeof (AMD_SMM_CPU_STATE) + sizeof (PROCESSOR_SMM_DESCRIPTOR) + gcSmiHandlerSize - 1);
  ...
  for (Index = 0; Index < gSmmCpuPrivate->SmmCoreEntryContext.NumberOfCpus; Index++) {
    ...
    gSmmCpuPrivate->CpuSaveStateSize[Index] = sizeof(AMD_SMM_CPU_STATE);
