  ...
  )
{
  ...
  // Only sync MTRRs for family 15
  AsmCpuid (0x80000001, &RegEax, NULL, NULL, NULL);
  if (((RegEax >> 20) & 0xFF) == 0x6) {
    //
    // Set default Mtrr
    //
    MsrValue = AsmReadMsr64 (0xc0010010);
    MsrValue |= 0x00080000;
    AsmWriteMsr64 (0xc0010010, MsrValue);

    AsmWriteMsr64 (0x00000268, 0x1010101010101010);
    AsmWriteMsr64 (0x00000269, 0x1010101010101010);
    AsmWriteMsr64 (0x0000026A, 0x1010101010101010);
    AsmWriteMsr64 (0x0000026B, 0x1010101010101010);
    AsmWriteMsr64 (0x0000026C, 0x1818181818181818);
    AsmWriteMsr64 (0x0000026D, 0x1818181818181818);
    AsmWriteMsr64 (0x0000026E, 0x1010101010101010);
    AsmWriteMsr64 (0x0000026F, 0x1010101010101010);
...
//  EnableDisableSVM (PcdGet8 (PcdSVMEnable),1);
  ...
  )
{
  ...
  mSvmEnabled = 1; //PcdGet8 (PcdSVMEnable);
  mSvmLock    = PcdGet8 (PcdSVMLock);
  AsmWbinvd ();
  ...
  )
{
  ...
  EFI_EVENT   SmmConfigurationNotificationEvent;
//   EFI_EVENT   ExitBootServiceEvent;
  ...
  //
  // Install notification callback on EFI_SMM_CONFIGURATION_PROTOCOL protocol.
  //
  SmmConfigurationNotificationEvent = EfiCreateProtocolNotifyEvent (
