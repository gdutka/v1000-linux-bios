...
/**
  Append VBIOS option in the VFR label

  @param[in] StartOpCodeHandle         Pointer to the opcode handle

  @retval EFI_SUCCESS                  Append VBIOS option successfully.
  @retval EFI_OUT_OF_RESOURCES         Failed to allocate opcode handle.
**/
STATIC
EFI_STATUS
AppendVbiosOption (
  ...
  )
{
  ...
  VideoOption = CommonGetVariableData (L"VBIOS", &gEfiGenericVariableGuid);
  if (VideoOption == NULL) {
  ...
  HiiCreateOneOfOptionOpCode (OptionsOpCodeHandle, STRING_TOKEN (STR_ONE_OF_PCI), gDeviceManagerPrivate.VideoBios ? 0 : EFI_IFR_OPTION_DEFAULT, EFI_IFR_NUMERIC_SIZE_1, 0);
  HiiCreateOneOfOptionOpCode (OptionsOpCodeHandle, STRING_TOKEN (STR_ONE_OF_AGP), gDeviceManagerPrivate.VideoBios ? EFI_IFR_OPTION_DEFAULT : 0, EFI_IFR_NUMERIC_SIZE_1, 1);

  HiiCreateSubTitleOpCode (StartOpCodeHandle, STRING_TOKEN (STR_EMPTY_STRING), 0, 0, 1);
...
{
  if (FormSetGuid != NULL && CompareGuid (FormSetGuid, &mDeviceManagerGuid)) {
    //
    // For old browser, have a chance to update root form when entering device manager.
    //
    DeviceManagerNotify ();
  ...
  )
{
  ...
  FormBrowser2->SendForm = DeviceManagerSendForm;
...
)
{
  ...
  if (NextShowFormId == DEVICE_MANAGER_FORM_ID) {
    AppendVbiosOption (StartOpCodeHandle);
  }
