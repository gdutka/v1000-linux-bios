  ...
  )
{
  ...
  if (mInSmram && mPrivate->AcpiTimer) {
    //
  ...
  )
{
  ...
  if (CpuSaveStatePackage->CpuIndex >= mSmst->NumberOfCpus) {
    return EFI_NOT_FOUND;
  }
  ...
  )
{
  ...
  if (Table->Signature == EFI_ACPI_3_0_FIXED_ACPI_DESCRIPTION_TABLE_SIGNATURE) {
    ...
    //
    // Process is done, unregister the callback function.
    //
    AcpiSdtProtocol->RegisterNotify (FALSE, AcpiFadtNotify);
  }
  return EFI_SUCCESS;
}
