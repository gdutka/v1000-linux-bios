  ...
  )
{
  ...
  //
  // If the FV header is corrupted, the FV HOB also may not be created because
  // It may check FV alignment failed. To prevent from this case, restore correct
  // FV header even if the FV HOB is not built.
  //
  if (!NvFvFound && GetBootModeHob () != BOOT_IN_RECOVERY_MODE &&
      ValidateFvHeader ((EFI_FIRMWARE_VOLUME_HEADER *)(UINTN)GetNvStoreBaseAddress ()) != EFI_SUCCESS) {
    Status = RestoreCorrectFvHeader (GetNvStoreBaseAddress ());
    if (!EFI_ERROR (Status)) {
      gRT->ResetSystem (EfiResetCold, EFI_SUCCESS, 0, NULL);
    }
  }
