  ...
  )
{
  ...
  Entry = VariableAllocateZeroBuffer (sizeof (*Entry) + MAX_VARIABLE_NAME_SIZE, TRUE);   
  if (Entry == NULL) {
  ...
  Name = (CHAR16 *) ((UINTN) Entry + sizeof (*Entry));
  StrnCpyS   (Name, MAX_VARIABLE_NAME_SIZE / sizeof (CHAR16), VariableName, StrLen (VariableName));
  ...
  )
{
  ...
  if (BufferSize != sizeof (SMM_VAR_BUFFER) + sizeof (LIST_ENTRY *) ||
