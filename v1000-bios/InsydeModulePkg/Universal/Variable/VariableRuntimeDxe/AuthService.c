...
/**
  ...
  by any valid count-based auth variable.

  @param[in] PubKeyIndex  Index of the public key in public key store.

  @retval TRUE            The PubKeyIndex is still in use.
  @retval FALSE           The PubKeyIndex is not referenced by any count-based auth variable.
**/
STATIC
BOOLEAN
IsValidPubKeyIndex (
...
--*/
{
  ...
  StrCpyS (GET_VARIABLE_NAME_PTR (Variable), (TotalBufferSize - sizeof (VARIABLE_HEADER)) / sizeof(CHAR16), GET_VARIABLE_NAME_PTR (PubKeyHeader));
  ...
  )
{
  ...
  if (!IsFound) {
    ...
    if (mPubKeyNumber == MAX_KEY_NUM) {
      //
      // Public key database is full, try to reclaim invalid key.
      //
  ...
  )
{
  ...
  if ((mPubKeyNumber == 0 && Variable.CurrPtr == NULL) || mPubKeyNumber >= MAX_KEY_NUM) {
    VarAttr       = EFI_VARIABLE_NON_VOLATILE | EFI_VARIABLE_RUNTIME_ACCESS | EFI_VARIABLE_BOOTSERVICE_ACCESS | EFI_VARIABLE_AUTHENTICATED_WRITE_ACCESS;
    VarValue      = 0;
    mPubKeyNumber = 0;
    Status        = UpdateVariable (
