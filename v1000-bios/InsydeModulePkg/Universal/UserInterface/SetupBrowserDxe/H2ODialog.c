  ...
  )
{
  ...
  if (DialogOperator != DlgOk) {
    ...
    StrCatS (String, (StrSize(ShowString) + sizeof(CHAR16)) / sizeof(CHAR16), L"?");
  } else {
    String = ShowString;
  }
  ...
  CleanLine = AllocatePool ((DialogWidth + 1) * sizeof (CHAR16));
  if (CleanLine == NULL) {
    return 0;
  }
  ...
  )
{
  ...
  do {
    ...
    DEBUG ((DEBUG_INFO, "OemKernelServices Call: OemSvcPasswordInputFilter \n"));
    OemSvcStatus = OemSvcPasswordInputFilter (&EfiKeyData);
    DEBUG ((DEBUG_INFO, "OemKernelServices OemSvcPasswordInputFilter Status: %r\n", OemSvcStatus));
  ...
  )
{
  ...
  StrLenth = StrLenth / 2;
  CleanLine = AllocatePool (((StrLenth + 2) * 2 + 2) * sizeof (CHAR16));
  if (CleanLine == NULL) {
    return EFI_OUT_OF_RESOURCES;
  }

  SelectIndexLin = AllocatePool (((StrLenth + 2) * 2 + 2) * sizeof (CHAR16));
  if (SelectIndexLin == NULL) {
    FreePool (CleanLine);
    return EFI_OUT_OF_RESOURCES;
  }
  ...
  )
{
  ...
  DrawDialogBlock (RequestedWidth, NumberOfLines, ArrayOfStrings, Marker);
  VA_END (Marker);
