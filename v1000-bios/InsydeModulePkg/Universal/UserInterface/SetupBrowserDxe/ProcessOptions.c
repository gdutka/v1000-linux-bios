  ...
  *OptionString = NULL;
      ...
      *OptionString = AllocateZeroPool (Question->MaxContainers * BufferSize);
      ...
      for (Index = 0; Index < Question->MaxContainers; Index++) {
        ...
        if (!Suppress) {
          ...
          NewStrCat (OptionString[0], ((UINTN)(Question->MaxContainers)) * BufferSize, Character);
          StringPtr = GetToken (OneOfOption->Text, Selection->Handle);
          if (StringPtr == NULL) {
            return EFI_NOT_FOUND;
          }
          NewStrCat (OptionString[0], ((UINTN)(Question->MaxContainers)) * BufferSize, StringPtr);
  ...
  case EFI_IFR_ONE_OF_OP:
    ...
    if (Selected) {
      ...
      *OptionString = AllocateZeroPool (BufferSize);
      ...
      if (!Suppress) {
        Character[0] = LEFT_ONEOF_DELIMITER;
        NewStrCat (OptionString[0], BufferSize, Character);
        StringPtr = GetToken (OneOfOption->Text, Selection->Handle);
        if (StringPtr == NULL) {
          return EFI_NOT_FOUND;
        }
        NewStrCat (OptionString[0], BufferSize, StringPtr);
        Character[0] = RIGHT_ONEOF_DELIMITER;
        NewStrCat (OptionString[0], BufferSize, Character);
  ...
  case EFI_IFR_STRING_OP:
    if (Selected) {
      ...
      Status = ReadString (MenuOption, gPromptForData, StringPtr, Maximum + 1);
      if (!EFI_ERROR (Status)) {
        ...
        *StringPtr = 0;
        ...
        if (EFI_ERROR (Status)) {
          ...
          if (Status != EFI_CRC_ERROR) {
            ...
            //
            Status = ReadPassword (MenuOption, ENTER_OLD_PASSWORD, LineNum++, TRUE, StringPtr, (Maximum + 1));
          } else {
            //
            // Unlock password
            //
            UnlockPassword = TRUE;
            Status = ReadPassword (MenuOption, UNLOCK_PASSWORD, LineNum++, TRUE, StringPtr, (Maximum + 1));
          }
...
//              PasswordInvalid ();
      ...
      Status = ReadPassword (MenuOption, ENTER_NEW_PASSWORD, LineNum++, !CheckOldPassword, StringPtr, Maximum + 1);
      if (EFI_ERROR (Status)) {
        //
      ...
      Status = ReadPassword (MenuOption, CONFIRM_NEW_PASSWORD, LineNum++, FALSE, TempString, Maximum + 1);
      if (EFI_ERROR (Status)) {
        //
        // Reset state machine for interactive password
        //
