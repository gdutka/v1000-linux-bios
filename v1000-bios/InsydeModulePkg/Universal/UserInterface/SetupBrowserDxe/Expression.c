  ...
  )
{
  ...
  StringPtrBufSize = StrSize (String[1]) + StrSize (String[0]);
  StringPtr= AllocatePool (StringPtrBufSize);
  ASSERT (StringPtr != NULL);
  if (StringPtr == NULL) {
    Status = EFI_OUT_OF_RESOURCES;
    return Status;
  }
  StrCpyS (StringPtr, StringPtrBufSize / sizeof(CHAR16), String[1]);
  StrCatS (StringPtr, StringPtrBufSize / sizeof(CHAR16), String[0]);
