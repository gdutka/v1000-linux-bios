    ...
    do {
      ...
      case CHAR_BACKSPACE:
        if (StringBuffer[0] != CHAR_NULL) {
          ...
          StrCpyS (StringBuffer, MaximumStringSize / sizeof(CHAR16), TempString);
        }
        NeedPrint = TRUE;
        break;
      ...
      default:
        ...
        if ((StringBuffer[0] == CHAR_NULL) && (Key.UnicodeChar != CHAR_BACKSPACE)) {
          ...
          KeyPad[1] = CHAR_NULL;
          StrCatS (StringBuffer, MaximumStringSize / sizeof(CHAR16), KeyPad);
          StrCatS (TempString, MaximumStringSize, KeyPad);
        }
        NeedPrint = TRUE;
        break;
      }
...
/**
 ...
 @param[in  Marker                  The variable argument list to get the opcode and associated attributes.
**/
VOID
  ...
  )
{
  ...
  VA_START (Marker, ArrayOfStrings);
  CreateSharedPasswordPopUp (RequestedWidth, PasswordPopUpType, MaxInputChar, NumberOfLines, ArrayOfStrings, Marker);
  VA_END (Marker);
}
  ...
  while (TRUE) {
    ...
    case CfUpdateHelpString:
      ...
      if ((Repaint || NewLine) && gClassOfVfr != EFI_GENERAL_APPLICATION_SUBCLASS) {
        ...
        //
        if (MenuOption == NULL || MenuOption->ThisTag->Help == 0 || !IsSelectable (MenuOption)) {
    ...
    case CfReadKey:
      ...
      if (mRepaintFormFlag && MenuOption != NULL) {
      ...
      if (gGraphicsEnabled &&
        ...
        if (Key.ScanCode == SCAN_LEFT || Key.ScanCode == SCAN_RIGHT) {
          ...
          for (Link = NewPos, Count = 0; (Link != NULL) && (Link != &Menu) && (Count < NumColumn);
      ...
      case '-':
        ...
        if ((Statement->Operand == EFI_IFR_DATE_OP)
          ...
          ScuSafeFreePool ((VOID **)&OptionString);
        } else if ( (Statement->Operand == EFI_IFR_ACTION_OP || Statement->Operand == EFI_IFR_CHECKBOX_OP) &&
                    ((Statement->QuestionFlags & (UINT8) EFI_IFR_CHECKBOX_DEFAULT_MFG) == (UINT8) EFI_IFR_CHECKBOX_DEFAULT_MFG)) {
          ...
          if (!EFI_ERROR (Status)) {
            ...
            if (Statement->Operand == EFI_IFR_CHECKBOX_OP) {
              ...
              Selection->QuestionId = MenuOption->ThisTag->QuestionId;
              Selection->Action     = UI_ACTION_REFRESH_FORM;
              Selection->CurrentRow = Key.UnicodeChar == '+' ? MenuOption->Row : MenuOption->Row + MenuOption->Skip;
              Selection->Statement  = NULL;
            }
          }
	  ...
	  // OemServices
	  //
          DEBUG ((DEBUG_INFO, "OemKernelServices Call: OemSvcInstallBootMangerKeyTable \n"));
          Status = OemSvcInstallBootMangerKeyTable (
                     &ScanCodeToOperationCount,
                     &CodeToOperation
                     );
          DEBUG ((DEBUG_INFO, "OemKernelServices OemSvcInstallBootMangerKeyTable Status: %r\n", Status));
    ...
    case CfUiSave:
      ...
      if (!EFI_ERROR (Status) && MenuOption != NULL) {
