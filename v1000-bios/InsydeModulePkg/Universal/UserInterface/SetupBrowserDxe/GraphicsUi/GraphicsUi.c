...
extern EFI_STRING_ID mHelpString[];
  ...
  )
{
  ...
  AttributesStrBufLen = 0x1000;
  AttributesStr = AllocatePool (sizeof (CHAR16) * AttributesStrBufLen);
  ...
  while (!IsNull (&Form->StatementListHead, Link)) {
    ...
    if (mFrontPage) {
      ...
      if (Operand == EFI_IFR_SUBTITLE_OP) {
        ...
        StrCpyS (AttributesStr, AttributesStrBufLen, L"name='menu_prompt' textpadding='5,2,5,2'");
        ...
        StrCpyS (AttributesStr, AttributesStrBufLen, L"textcolor='0xFF008287' fontsize='40'");
        Prompt = CreateControl (L"Label", PromptString, Layout, Manager);
        UiApplyAttributeList (Prompt, AttributesStr);
        SetWindowLongPtr (Prompt->Wnd, GWLP_USERDATA, (INTN)Statement);
        ((UI_CONTAINER *)Layout)->Add ((UI_CONTAINER *)Layout, Prompt);
