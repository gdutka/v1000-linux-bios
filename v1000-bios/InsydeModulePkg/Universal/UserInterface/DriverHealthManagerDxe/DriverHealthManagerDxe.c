...
{
  if (FormSetGuid != NULL && CompareGuid (FormSetGuid, &gDeviceManagerFormSetGuid)) {
    //
    // For old browser, have a chance to update root form when entering driver health manager.
    //
    DriverHealthManagerUpdateForm (FALSE);
  }
  ...
  )
{
  ...
  FormBrowser2->SendForm = DriverHealthManagerSendForm;
