  ...
  )
{
  ...
  if (RecordStrings[0] != '\0') {
    ...
    for (StrIndex = 0; StrIndex < StrCount; ++StrIndex) {
      ...
      if (OffsetTable != NULL) {
        ...
        DEBUG ((DEBUG_INFO, "OemKernelServices Call: OemSvcDxeGetSmbiosReplaceString \n"));
        Status = OemSvcDxeGetSmbiosReplaceString(Hdr->Type, OffsetTable->Offset[StrIndex], &DataInfo.Length, &DataInfo.Data);
        DEBUG ((DEBUG_INFO, "OemKernelServices OemSvcDxeGetSmbiosReplaceString Status: %r\n", Status));
  ...
  //
  DEBUG ((DEBUG_INFO, "OemKernelServices Call: OemSvcDxeUpdateSmbiosRecord \n"));  
  Status = OemSvcDxeUpdateSmbiosRecord((EFI_SMBIOS_TABLE_HEADER *)Buffer);
  DEBUG ((DEBUG_INFO, "OemKernelServices OemSvcDxeUpdateSmbiosRecord Status: %r\n", Status));
  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "OemKernelServices Call: OemSvcDxeInstallPnpStringTable \n"));
  Status = OemSvcDxeInstallPnpStringTable (&TableCount, &PnpTable);
  DEBUG ((DEBUG_INFO, "OemKernelServices OemSvcDxeInstallPnpStringTable Status: %r\n", Status));
