  ...
  )
{
  ...
  if (mOutStack == NULL) {
    mOutStack = AllocateZeroPool (H2O_GIF_OUT_STACK_MAX_LEN * sizeof (UINT8));
  }

  if (mLzwTable == NULL) {
    mLzwTable = AllocateZeroPool (H2O_GIF_LZW_TABLE_MAX_LEN * sizeof (LZW_ENTRY));
  }
  ...
  for (;;) {
    ...
    ImageData = (EFI_GRAPHICS_OUTPUT_BLT_PIXEL *)AllocatePool ((UINTN) ImageSize);
    if (ImageData == NULL) {
      Status = EFI_OUT_OF_RESOURCES;
      goto Err;
    }
